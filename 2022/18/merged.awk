...

{
    if (NR == 1) {
        ...
        if ($1 SUBSEP $2+1 SUBSEP $3 in cubes) { cubes[$1,$2,$3]--; cubes[$1,$2+1,$3]-- }
        if ($1 SUBSEP $2 SUBSEP $3-1 in cubes) { cubes[$1,$2,$3]--; cubes[$1,$2,$3-1]-- }
        if ($1 SUBSEP $2 SUBSEP $3+1 in cubes) { cubes[$1,$2,$3]--; cubes[$1,$2,$3+1]-- }
    }
}
...
END {
    ...
    for (x=minx;x<=maxx;x++)
        for (y=miny;y<=maxy;y++)
            for (z=minz;z<=maxz;z++)
                if (!(x SUBSEP y SUBSEP z in cubes)) {
                    if (x-1 SUBSEP y SUBSEP z in cubes) { if (cubes[x-1,y,z] > 0) cubes[x-1,y,z]-- }
                    if (x+1 SUBSEP y SUBSEP z in cubes) { if (cubes[x+1,y,z] > 0) cubes[x+1,y,z]-- }
                    if (x SUBSEP y-1 SUBSEP z in cubes) { if (cubes[x,y-1,z] > 0) cubes[x,y-1,z]-- }
                    if (x SUBSEP y+1 SUBSEP z in cubes) { if (cubes[x,y+1,z] > 0) cubes[x,y+1,z]-- }
                    if (x SUBSEP y SUBSEP z-1 in cubes) { if (cubes[x,y,z-1] > 0) cubes[x,y,z-1]-- }
                    if (x SUBSEP y SUBSEP z+1 in cubes) { if (cubes[x,y,z+1] > 0) cubes[x,y,z+1]-- }
                }
