...
function trace(x,y,dir) {
    ...
    energized[x,y][dir]
    c=world[y][x]
    if (c=="/")       return step(x,y,1+4*(dir>1)-dir)
    else if (c=="\\") return step(x,y,3-dir)
    else if (c=="-" && (dir==0 || dir==2)) { step(x,y,3); return step(x,y,1) }
    else if (c=="|" && (dir==1 || dir==3)) { step(x,y,0); return step(x,y,2) }
    return step(x,y,dir)
}
...
END {
    ...
    for (x=1;x<=length(world[1]);x++) { run(x,1,2); run(x,NR,0) }
    for (y=1;y<NR;y++) { run(1,y,1); run(length(world[1]),y,3) }
    print "Part 2: " part2
}
