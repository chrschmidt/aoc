...
function distance2(a, b, pointa, pointb) {
    split(a, pointa, ",")
    split(b, pointb, ",")
    return ((pointa[1]-pointb[1])^2 + (pointa[2]-pointb[2])^2 + (pointa[3]-pointb[3])^2)
}
...
END {
    for (sid in input) {
        for (pa in input[sid])
            for (pb in input[sid])
                distances[sid][pa,pb] = distance2(input[sid][pa],input[sid][pb])
    }
